...
open_re_terminal() {
	...
	if command -v niri >/dev/null 2>&1 && niri msg version >/dev/null 2>&1; then
		focus_re_workspace
		if niri msg action spawn -- alacritty -t "${title}" -e "${SCRIPT_DIR}/re-avd.sh" attach >/dev/null 2>&1; then
			return 0
		fi
	...
	if [[ -n "${DISPLAY:-}" || -n "${WAYLAND_DISPLAY:-}" ]]; then
		nohup alacritty -t "${title}" -e "${SCRIPT_DIR}/re-avd.sh" attach >/dev/null 2>&1 &
