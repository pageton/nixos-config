...
open_re_terminal() {
	...
	if open_herdr_space "${title}" "${SCRIPT_DIR}/web-re.sh attach"; then
	...
	if command -v niri >/dev/null 2>&1 && niri msg version >/dev/null 2>&1; then
		focus_re_workspace
		if niri msg action spawn -- alacritty --title "${title}" -e "${SCRIPT_DIR}/web-re.sh" attach >/dev/null 2>&1; then
			return 0
		fi
	...
	if [[ -n "${DISPLAY:-}" || -n "${WAYLAND_DISPLAY:-}" ]]; then
		nohup alacritty --title "${title}" -e "${SCRIPT_DIR}/web-re.sh" attach >/dev/null 2>&1 &
