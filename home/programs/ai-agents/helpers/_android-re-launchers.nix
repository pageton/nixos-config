...
let
  ...
  mkForgeAndroidReLauncher =
    ...
    pkgs.writeShellScriptBin name ''
      ...
      # Launch forge interactively — the android-re prompt is in AGENTS.md under the forge profile dir.
      title="android-re (forge)"
      if [[ "${forgeProfile}" != "forge" ]]; then
        title="android-re (forge/${forgeProfile})"
      fi

      if command -v alacritty >/dev/null 2>&1; then
        FORGE_CONFIG="$HOME/.${forgeProfile}" \
          exec alacritty --title "$title" -e forge "$@"
      else
        FORGE_CONFIG="$HOME/.${forgeProfile}" \
          exec forge "$@"
      fi
  ...
  mkClaudeAndroidReLauncher =
    ...
    pkgs.writeShellScriptBin name ''
      ...
      if command -v alacritty >/dev/null 2>&1; then
        alacritty --title "android-re (claude)" -e claude \
...
++ map mkForgeAndroidReLauncher [
  ...
  {
    name = "fgglmare";
    forgeProfile = "forge-glm";
  }
  {
    name = "fggemare";
    forgeProfile = "forge-gemini";
  }
  {
    name = "fggptare";
    forgeProfile = "forge-gpt";
  }
  {
    name = "fgorare";
    forgeProfile = "forge-openrouter";
  }
  {
    name = "fgsare";
    forgeProfile = "forge-sonnet";
  }
  {
    name = "fgzenare";
    forgeProfile = "forge-zen";
  }
]
