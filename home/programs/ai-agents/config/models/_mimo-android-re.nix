# Android RE agent definition for MiMoCode.
# Mirrors _opencode-android-re.nix; mimo shares opencode's agent schema.
# The launcher pins default_agent=android-re per-session via MIMOCODE_CONFIG_DIR overlay.
...
{
  "android-re" = {
    # Mimo-available default; the launcher's `-m <profile-model>` overrides per-session.
    model = models.mimo-pro;
    ...
    prompt = ''
      ...
      ## Bash scripts (all run from repo root /home/sadiq/System)
      ...
      scripts/ai/android-re/mimo-android-re.sh — MiMoCode launcher (used by mi*are aliases)
      ...
      - The emulator boots via `re-avd.sh start-basic` (emulator-only baseline). Device spoofing, Frida, mitm, and proxy are NOT automatic — run `re-avd.sh spoof`, `re-avd.sh frida-start`, and `re-avd.sh proxy-set` on demand.
