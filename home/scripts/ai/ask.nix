{pkgs, ...}:
pkgs.writeShellScriptBin "ai-ask" ''
  # ai-ask - AI API client using z.ai OpenAI-compatible API
  # Usage: ai-ask [-d] [-s "system prompt"] "Your prompt here"
  ...
  show_usage() {
      echo "Usage: ai-ask [-d] [-c] [-s \"system prompt\"] \"Your prompt here\""
      echo ""
      echo "Options:"
      ...
      echo ""
      echo "Examples:"
      echo "  ai-ask \"Write a Python function to calculate factorial\""
      echo "  ai-ask -d \"Explain quantum computing in simple terms\""
      echo "  ai-ask -c \"Summarize this text\""
      echo "  ai-ask -s \"You are a helpful assistant\" \"What is bash?\""
      echo ""
  ...
  if command -v ${pkgs.jq}/bin/jq >/dev/null 2>&1; then
      ...
      CLEAN_RESPONSE=$(echo "$RESPONSE" | ${pkgs.jq}/bin/jq -r '.choices[0].message.content' 2>/dev/null) || {
  ...
  if [[ "$COPY_TO_CLIPBOARD" == "true" ]]; then
      if command -v ${pkgs.wl-clipboard}/bin/wl-copy >/dev/null 2>&1; then
          echo "$CLEAN_RESPONSE" | ${pkgs.wl-clipboard}/bin/wl-copy
      else
