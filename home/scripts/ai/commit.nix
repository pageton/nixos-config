...
pkgs.writeShellScriptBin "ai-commit" ''
  ...
  analyze_changes() {
      ...
      if [[ "$diff_size" -gt 1000 ]]; then
          ...
          local diff_numstat
          diff_numstat="$(GIT_PAGER=cat ${pkgs.git}/bin/git diff --cached --numstat | head -15 | awk '{printf "  %s: +%s -%s lines\n", $3, $1, $2}')"
          echo "$diff_numstat"
      else
          # Use stat instead of full diff to avoid bat opening
          local diff_summary
          diff_summary="$(GIT_PAGER=cat ${pkgs.git}/bin/git diff --cached --stat)"
          echo "$diff_summary"
      fi
  }
